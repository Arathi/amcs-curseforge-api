...
import { CurseForgeApi, ModLoaderType } from "../src";
...
describe("CurseForgeApi", async () => {
  ...
  test("获取指定模组", async () => {
    const resp = await api.getMod(MOD_ID_JEI);
    const jei = resp.data;
  ...
  test("获取指定模组文件", async () => {
    const resp = await api.getModFile(MOD_ID_JEI, MOD_FILE_ID_JEI);
  ...
  test("获取模组文件下载地址", async () => {
    ...
    const resp = await api.getModFileDownloadURL(modId, fileId);
